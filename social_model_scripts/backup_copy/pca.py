...
# Note: This script now works with latent representations with no velocity features and test depth output
...
pca_output_folder = '/media/fenglab/newssd/social/'
...
def main():
    ...
    output_folder = f'{pca_output_folder}bottom_playing_and_alone_pca_seq_{sequence_length}_latent_{latent_dim}_bottleneck_{bottleneck_dim}_no_velocity_test_depth/'
    
    print(f"Starting PCA analysis for subset latent space ({desired_categories})...")
    ...
    # Save PCA vectors
    pca_vectors_file = os.path.join(pca_output_dir, f'pca_vectors_{"Bottom_Playing_and_Alone"}_n_components_{n_components}_no_velocity_test_depth_sequence_length_{sequence_length}.npy')
    np.save(pca_vectors_file, pca_vectors)
    print(f"Saved PCA vectors to: {pca_vectors_file}")
    
    # Save PCA model
    pca_model_file = os.path.join(pca_output_dir, f'pca_model_{"Bottom_Playing_and_Alone"}_n_components_{n_components}_no_velocity_test_depth_sequence_length_{sequence_length}.pkl')
    with open(pca_model_file, 'wb') as f:
        pickle.dump(pca, f)
    print(f"Saved PCA model to: {pca_model_file}")
    
    # Save frame indices for reference
    frame_indices_file = os.path.join(pca_output_dir, f'frame_indices_{"Bottom_Playing_and_Alone"}_no_velocity_test_depth_sequence_length_{sequence_length}.npy')
    ...
    if os.path.exists(mapping_file):
        ...
        mapping_save_file = os.path.join(pca_output_dir, f'frame_indices_mapping_{"Bottom_Playing_and_Alone"}_no_velocity_test_depth_sequence_length_{sequence_length}.pkl')
