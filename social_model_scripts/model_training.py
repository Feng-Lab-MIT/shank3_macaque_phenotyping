...
# Global parameters
# Paths - set these or override via pipeline config
folder = '/path/to/model_output/'
# Category folder name for output subfolders (pipeline can overwrite)
category_folder_name = 'all_categories'
data_path = '/path/to/all_dfs.pkl'
category_file = '/path/to/category_file.csv'
# Path for monkey/day mapping: CSV whose columns (with '_') give dataframe identifiers
sample_feature_path = '/path/to/sample_feature.csv'
...
def preprocess_data_with_relative_features_subset(all_dfs, sequence_length, prediction_length, filtered_df, original_df_indices):
    ...
    # Generate di_mp_dic mapping from sample feature (DataFrame Index -> monkey_day string)
    sample_feature = pd.read_csv(sample_feature_path, index_col=[0], header=[0])
    monkey_day_list = sorted([col for col in sample_feature.columns if '_' in col])
...
def resume_training_from_checkpoint(checkpoint_path, train_data, val_data, sequence_length, prediction_length,
                                  model_params, additional_feature_indices, folder,
    ...
    callbacks = [
        ...
        ModelCheckpoint(
                filepath=f'{folder}best_model_relative_features_with_depth_{category_folder_name}_decoder_fixed_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}_latent{latent_dims[0]}_bottleneck{bottleneck_dims[0]}.h5',
            monitor='val_loss',
            save_best_only=True,
            save_weights_only=False
        ),
        CSVLogger(
            filename=f'{folder}training_log_resumed_{category_folder_name}_relative_with_depth_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}_latent{latent_dims[0]}_bottleneck{bottleneck_dims[0]}.csv',
            separator=',',
            append=False
        )
    ]
...
def train_model(train_data, val_data, sequence_length, prediction_length, model_params, additional_feature_indices):
    """
    ...
    callbacks = [
        ...
        CSVLogger(
            filename=f'{folder}training_log_{category_folder_name}_relative_with_depth_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}_latent{latent_dims[0]}_bottleneck{bottleneck_dims[0]}.csv',
            separator=',',
            append=False
        )
    ]
...
def main():
    ...
    """
    print("Loading data...")
    all_dfs = pd.read_pickle(data_path)
    ...
    print(f"\n{'='*60}")
    print("TRAINING MODEL (all categories)")
    print(f"{'='*60}")
    
    train_model_for_categories(all_dfs, category_df)

def train_model_for_categories(all_dfs, category_df):
    """
    Train model using all behavioral categories.
    """
    filtered_df = category_df.copy()
    print(f"Using all categories: {len(filtered_df)} frames")
    print(f"Will process {len(filtered_df)} frames")
    ...
    print("\n=== MODEL SUMMARY ===")
    model, history = train_model(train_data, val_data, sequence_length, prediction_length, model_params, additional_feature_indices)
    
    # Save final model with category information in filename
    model_filename = f'{folder}final_model_relative_features_with_depth_{category_folder_name}_decoder_fixed_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}.h5'
    ...
    feature_info_filename = f'{folder}feature_info_relative_features_with_depth_{category_folder_name}_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}.pkl'
    ...
    mapping_filename = f'{folder}frame_indices_mapping_{category_folder_name}_relative_with_depth_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}.pkl'
...
def resume_training_example():
    ...
    Set data_path, category_file, and folder at the top of this script before running.
    """
    sequence_length = sequence_lengths[0]
    latent_dim = latent_dims[0]
    bottleneck_dim = bottleneck_dims[0]
    checkpoint_path = f'{folder}best_model_relative_features_with_depth_{category_folder_name}_decoder_fixed_v4_enhanced_standardized_no_velocity_test_depth_output_seq{sequence_length}_latent{latent_dim}_bottleneck{bottleneck_dim}.h5'
