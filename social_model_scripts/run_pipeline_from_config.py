...
def get_category_string(categories):
    """Get category string for file/folder naming. Used when desired_categories is 'all' or a list."""
    if categories == 'all' or (isinstance(categories, list) and 'all' in categories):
        return 'all_categories'
    if isinstance(categories, list) and len(categories) > 0:
        return '_and_'.join(sorted(c.replace(' ', '_').lower() for c in categories))
    return 'all_categories'
...
def main():
    ...
    if pipeline_cfg['run_training']:
        ...
        category_folder_name = get_category_string(categories_cfg['desired_categories'])
        update_module_params(model_training, {
            ...
            'data_path': paths_cfg['data'],
            'category_file': paths_cfg['category_file'],
            'sample_feature_path': paths_cfg.get('sample_feature', '/path/to/sample_feature.csv')
        })
    ...
    if pipeline_cfg['run_latent_extraction']:
        latent_dir = f"{paths_cfg['latent_output']}{category_folder_name}_latent_seq_{model_cfg['sequence_length']}_latent_{model_cfg['latent_dim']}_bottleneck_{model_cfg['bottleneck_dim']}_no_velocity_test_depth/"
        print(f"  Latent: {latent_dir}")
    if pipeline_cfg['run_pca']:
        pca_dir = f"{paths_cfg['pca_output']}{category_folder_name}_pca_seq_{model_cfg['sequence_length']}_latent_{model_cfg['latent_dim']}_bottleneck_{model_cfg['bottleneck_dim']}_no_velocity_test_depth/"
        print(f"  PCA: {pca_dir}")
    if pipeline_cfg['run_tsne']:
        tsne_dir = f"{paths_cfg['tsne_output']}{category_folder_name}_tsne_seq_{model_cfg['sequence_length']}_latent_{model_cfg['latent_dim']}_bottleneck_{model_cfg['bottleneck_dim']}_no_velocity_test_depth/"
