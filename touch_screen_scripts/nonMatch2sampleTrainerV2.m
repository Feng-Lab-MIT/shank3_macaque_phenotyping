function nonMatch2sampleTrainerV2(setupID,monkeyID)
...
CONFIG.play_sound = 0;
CONFIG.total_trials = 960;
CONFIG.sampleTouchBan = false; % true touch sample is error
CONFIG.pre = false; % true only 1 distractor, false distractor number will increase
...
if CONFIG.show_map_objs
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{9}{i,1});
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
...
%         TRIALS(mod(total_trials - 1, CONFIG.n_trials_to_save) + 1) = TRIAL;
    ...
    if (~exist('TRIAL','var') || ~CONFIG.repeat_trial_if_error || (CONFIG.repeat_trial_if_error && ~TRIAL.error_type))
        ...
        sublist_objs_idx = randperm(size(list_objs, 1), CONFIG.n_distractors + 2);
        ...
        TRIAL.targetID = [sublist_objs_idx(target_pos) sublist_objs_idx(end)];
        ...
        for i = 1 : CONFIG.n_distractors + 1
            coords_objs_sample{i} = coords_all_objs{idx(i)}{i};
        end
