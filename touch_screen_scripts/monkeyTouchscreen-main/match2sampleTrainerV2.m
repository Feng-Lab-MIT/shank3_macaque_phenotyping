function match2sampleTrainerV2(setupID,monkeyID)

% % History:
% coded based on match_2_sample.
% 20-Aug-2022 modified from match2sampleTrainer
...
CONFIG.play_sound = 0;
CONFIG.total_trials = 960;%320;
CONFIG.sampleTouchBan = false; % true touch sample is error
% CONFIG.pre = true; % true only 1 distractor, false distractor number will increase
CONFIG.pre = false;
CONFIG.FlickerPunishment = 5; % flicker punishment time in seconds
%%% MAIN %%%
...
if CONFIG.show_map_objs
    ...
    color_text = palette{contains({palette{:,1}},'red'),2};
    ...
    % Rect touches
    Screen('FillRect', w, palette{contains({palette{:,1}},'white'),2}, coords_all_objs{2});
    for i = 1 : CONFIG.n_distractors + 1
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    WaitSecs(0.5);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{3}{i,1}, 1);
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{4}{i,1}, 1);
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{5}{i,1});
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{6}{i,1});
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{7}{i,1});
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, color, coords_all_objs{8}{i,1});
        Screen('DrawText', w, sprintf('%d', i), coords_all_objs{3}{i, 1}(1,1) + radius_square - 30 * CONFIG.scale_touchscreen, ...
            coords_all_objs{3}{i, 1}(1,2) + radius_square - 30 * CONFIG.scale_touchscreen, color_text);
    end
    Screen('Flip', w);
    ...
    function process_touch_event()
        ...
        if ~isempty(blob) && blob.mul > 0.1
            ...
            Screen('DrawDots', w, [blob.x, blob.y], ...
                blob.mul * baseSize, palette{contains({palette{:,1}},'white'),2}, [], 1, 1);
        else
...
%         fid = fopen(txtpath,'w');
        ...
        TRIALS_filenames = {};
        SESSION.TRIALS = [];
        color_rect_touch = [palette{contains({palette{:,1}},'white'),2} 0.3];
        evt_count = 0;
        first_event_id = [];
        buttonstate = 0;
        blackBG = true;
...
%         break;
        ...
        save(filepath,'trials','timer_session_start');
    end
...
%         TRIALS(mod(total_trials - 1, CONFIG.n_trials_to_save) + 1) = TRIAL;
    ...
    if (~exist('TRIAL','var') || ~CONFIG.repeat_trial_if_error || (CONFIG.repeat_trial_if_error && ~TRIAL.error_type))
        ...
        sublist_objs_idx = randperm(size(list_objs, 1), CONFIG.n_distractors + 1);
        ...
        coords_objs_this_trial = cell(1, CONFIG.n_distractors + 1);
        for i = 1 : CONFIG.n_distractors + 1
            coords_objs_this_trial{i} = coords_all_objs{idx(i)}{i};
        end
    end
...
%                 TRIAL.touch_log(end, 4) = CONFIG.main_display_area(4) - evt.MappedY;
    ...
    Screen('FillRect', w, palette{contains({palette{:,1}},'gray'),2}, rect);
    ...
    Screen('FillPoly', w, palette{contains({palette{:,1}},'red'),2}, coords_all_objs{1}, 1); % onset polygon / circle in the middle
    ...
    while ~TRIAL.error_type && ~TRIAL.sample_period_successful
        ...
        if ~isempty(blob) && ~isempty(evt) && (evt.Type == 2 || evt.Type == 3) && GetSecs >= sample_period_start_abs + CONFIG.time_inbetween_touches
            ...
            if GetSecs >= timer_inbetween_touches + CONFIG.time_inbetween_touches
                ...
                if evt_count > 0 && (CONFIG.sampleTouchBan || ~blob_inside_target)
                    if CONFIG.play_sound
                        play_sound_effect(PAHANDLES.fail. CONFIG);
                    end
                    flicker_screen_punishment(w, CONFIG);
%                     blob_inside_target = IsInRect(blob.x, blob.y, CenterRect(coords_all_objs{2}(1:4), rect));  % rect touch
                    blob_inside_target = IsInRect(blob.x, blob.y, coords_all_objs{2}(1:4,end));
                    if blob_inside_target
                        TRIAL.error_type = 4;
                        TRIAL.error_msg = 'touch target in sample period';
...
%                     fprintf('Touch outside target!\nTrials: %d/%d\nSuccess rate: %.2f%%\n', successful_trials, total_trials, 100 * successful_trials / (total_trials));
    ...
    % Draw all the objects
    for i = 1 : CONFIG.n_distractors + 1
        Screen('FillPoly', w, list_objs{contains({list_objs{:,1}},sublist_objs{i}),3}, coords_objs_this_trial{i});
    end
    if CONFIG.show_touch_area
...
%                         fprintf('Touch outside target!\nTrials: %d/%d\nSuccess rate: %.2f%%\n', successful_trials, total_trials, 100 * successful_trials / (total_trials));
                ...
                if evt.Type == 4 && blob_inside_target
                    ...
                    if CONFIG.arduino_connected
                        ...
                        Screen('DrawText', w, 'Correct!', CONFIG.main_display_area(3)/2 - 180 * CONFIG.scale_touchscreen, CONFIG.main_display_area(4)/2 - 250 * CONFIG.scale_touchscreen - CONFIG.half_side_square, palette{contains({palette{:,1}},'red'),2});
                        Screen('Flip', w);
                    end
